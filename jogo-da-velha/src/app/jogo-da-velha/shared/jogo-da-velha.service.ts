...
export class JogoDaVelhaService {
  ...
  inicializar(): void {
    ...
    this.vitoria = false;
    this.inicializaTabuleito();
  }

  inicializaTabuleito(): void {
  ...
  fimJogo(linha: number, coluna: number, tabuleiro: any, jogador: any) {
    let fim: any = false;
  ...
  obterJogada(jogador: number): number[] {
    ...
    for (let i = 0; i < this.TAM_TAB; i++) {
      for (let j = 0; i < this.TAM_TAB; j++) {
        ...
        if (this.fimJogo(i, j, tab, jogador)) {
          return [i, j];
        }
        tab[i][j] = this.VAZIO;
      }
